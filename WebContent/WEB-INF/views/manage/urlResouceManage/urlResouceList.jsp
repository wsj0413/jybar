...
<body>
    ...
    <form id="fm" action="${ctx}/manage/url/list.do?currPage=1" method="post">
	    <table class="tbform">
	        <thead>
	            <tr>
	        ...
	        <tbody>
	            ...
	            <tr>
	                ...
	                <td class="tdl">导航类型 </td>
	                <td>
	                    <select name="urltype" id="urltype" class="ipt" >
         ...
         <tbody>
            ...
            <c:forEach var="url" items="${urlList}" varStatus="status">
            	 <tr>
	                ...
	                <td>${url.url} </td>
	                <td>
						<c:if test="${url.urltype==1}">安卓交友</c:if>
						<c:if test="${url.urltype==2}">苹果交友</c:if>
						<c:if test="${url.urltype==3}">异性交友</c:if>
						<c:if test="${url.urltype==4}">知己交友</c:if>
						<c:if test="${url.urltype==5}">大学交友</c:if>
						<c:if test="${url.urltype==6}">休闲交友</c:if>
						<c:if test="${url.urltype==7}">老人交友</c:if>
						<c:if test="${url.urltype==8}">国际交友</c:if>
					</td>
	                <td>
					...
					<td>
						<c:if test="${url.isshowindex==0}">是</c:if>
						<c:if test="${url.isshowindex==1}">否</c:if>
					</td>
					<td>${url.sortweight} </td>
					<td>
						<c:if test="${url.isred==0}">标红</c:if>
						<c:if test="${url.isred==1}">不标红</c:if>
					</td>
					<td>${url.clickcount} </td>
					<td>${url.clickcounttrue} </td>
					<td>${url.clickorder} </td>
	  ...
	  if(urlstatus!=null && ""!=urlstatus){
       ...
       $('#update').click(function (){
           ...
           if(dataIdItem.length!=1){
  			 ...
  			 window.location.href="${ctx}/manage/url/update.do?tid="+dataIdItem[0];
  		   }
	  			...
	  			$('body').alert({
                    ...
                    buttons: [{
                        ...
                        callback: function () {
                        	$.ajax({
         		 			   type: "post",
         		 			   url: "${ctx}/manage/url/delete.do?tid="+dataIdItem,
       ...
       $('#add').click(function () {
           window.location.href="${ctx}/manage/url/add.do";
        });
...
function jump(curr){
	var fm=$("#fm");
	fm.attr("action", "${ctx}/manage/url/list.do?currPage="+curr);
	$("#fm").submit();
}
</script>	
</html>