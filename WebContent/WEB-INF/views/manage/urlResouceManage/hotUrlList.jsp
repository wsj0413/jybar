...
<body>
    <div class="alert alert-info">当前位置<b class="tip"></b>导航资源管理<b class="tip"></b>热门导航管理</div>
    <form id="fm" action="${ctx}/manage/hotUrl/list.do?currPage=1" method="post">
	    <table class="tbform">
	        <thead>
	            <tr>
	  ...
	  if(urlstatus!=null && ""!=urlstatus){
       ...
       $('#update').click(function (){
           ...
           if(dataIdItem.length!=1){
  			 ...
  			 window.location.href="${ctx}/manage/hotUrl/update.do?tid="+dataIdItem[0];
  		   }
	  			...
	  			$('body').alert({
                    ...
                    buttons: [{
                        ...
                        callback: function () {
                        	$.ajax({
         		 			   type: "post",
         		 			   url: "${ctx}/manage/hotUrl/delete.do?tid="+dataIdItem,
       ...
       $('#add').click(function () {
           window.location.href="${ctx}/manage/hotUrl/add.do";
        });
...
function jump(curr){
	var fm=$("#fm");
	fm.attr("action", "${ctx}/manage/hotUrl/list.do?currPage="+curr);
	$("#fm").submit();
}
</script>	
</html>