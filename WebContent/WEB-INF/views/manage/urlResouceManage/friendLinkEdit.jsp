...
<body>
    <div class="alert alert-info">当前位置<b class="tip"></b>导航资源管理<b class="tip"></b>友情资源管理</div>
	<form id="fm" action="${ctx}/manage/friendLink/editFri.do" method="post" onsubmit="return checkForm();">
	    <table class="table table-striped table-bordered table-condensed list">
	        <thead>
	            <tr>
	                <td colspan="4"><b>友情信息</b></td>
	            </tr>
	        </thead>
	        <tbody>
	            <tr>
	                <td width="15%">文本<font color="FF0000">*</font></td>
	                <td width="15%">
                		<input name="tcontent" id="tcontent" value="${fri.tcontent}" type="text" />
	                    <input name="tid" id="tid" value="${fri.tid}" type="hidden" />
	                    <input name="focusType" id="focusType" value="" type="hidden" />
	                    <input name="addType" id="addType" value="" type="hidden" />
	                    
	                </td>
	                <td width="15%">资源类型<font color="FF0000">*</font></td>
	                <td width="15%">
	                     <select name="astructure" id="astructure">
	                     	 <c:choose>
									<c:when test="${fri.ttype==1 }">
										 <option selected="selected" value="1" >友情链接</option>
										 <option value="2">合作伙伴</option>
										 <option value="3">赞助商</option>
									</c:when>
									<c:when test="${art.ttype==2 }">
